...
source("scripts_v3/fn_derived.R") # load main function to calculate derived vars
  ...
  for (dom in doms) {
    ...
    for (i in seq_len(nrow(tb_models))) {
      ...
      tb_files_mod %>%
        future_pwalk(function(file, t_i, t_f, ...) {
          ...
          if (str_detect(var_input, "wetbulb")) {
            ...
            # split annually
            system(
              str_glue("cdo splityear {f} {dir_raw_data}/{v}_yrsplit_"),
              ignore.stdout = T,
              ignore.stderr = T
            )
      ...
      future_walk(v, function(vv) {
        ...
        system(
          str_glue("cdo -settunits,d  -cat {ff} {dir_cat}/{vv}_cat.nc"),
          ignore.stdout = T,
          ignore.stderr = T
        )
      })
