land <- function() {
  ...
  land_r <-
    land_r %>%
    st_warp(s_proxy, method = "max", use_gdal = T) %>%
    setNames("land")
...
mosaic <- function(output_var, output_var_unit, years, prefix, dir_dest_cloud) {
  message(str_glue(
    "    mosaicking var {which(output_var == output_vars)} / {length(output_vars)} ({output_var})"
  ))
  ...
  years |>
    walk(\(yr) {
      ...
      f_res <- str_glue(
        "{dir_res}/{prefix}_{str_replace_all(output_var, '_', '-')}_yr_{yr}-01-01.nc"
      )
      ...
      system(
        str_glue("gcloud storage mv {f_res} {dir_dest_cloud}"),
        ignore.stdout = T,
        ignore.stderr = T
      )
    })
