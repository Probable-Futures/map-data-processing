  ...
  if (!is.null(land)) {
    df <-
      ...
      dplyr::mutate(
        ...
        land = purrr::pmap_dfr(
          df,
          function(start_x, end_x, start_y, end_y, ...) {
            ...
            pol_tile %>%
              dplyr::mutate(
                land = dplyr::if_else(all(is.na(dplyr::pull(land_tile))), F, T)
              )
          }
        )
      ) |>
...
rt_tile_loop <- function(df_tiles, list_files, FUN, dir_tiles) {
  ...
  furrr::future_pwalk(
    df_tiles,
    function(tile_id, start_x, start_y, count_x, count_y, ...) {
            ...
            if (land == FALSE) {
              ...
              if (is.null(time_dim)) {
                ...
                empty_array <-
                  array(
                    NA,
                    dim = c(dim(ss)[1], dim(ss)[2], time = length(time_dim))
                  )
              }
              ...
              stars::st_dimensions(empty_stars)[1] <- stars::st_dimensions(ss)[
                1
              ]
              stars::st_dimensions(empty_stars)[2] <- stars::st_dimensions(ss)[
                2
              ]
...
rt_tile_mosaic_gdal <- function(
  ...
  if (!is.null(time_dim)) {
    ...
    time_pos <-
      c(
        which(time_full == dplyr::first(time_dim)),
