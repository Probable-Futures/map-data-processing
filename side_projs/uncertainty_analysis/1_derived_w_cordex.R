...
doms <- c("SEA", "CAS", "WAS", "EAS", "AFR", "EUR", "NAM", "CAM", "SAM", "AUS") |> tail(-1) # *******
...
for(dom in doms) {
  ...
  for(rcm in rcms) {
    ...
    for(gcm in gcms) {
      ...
      dim_1 <- st_get_dimension_values(s_proxy, 1)
      dim_2 <- st_get_dimension_values(s_proxy, 2)
      ...
      df_tiles <- rt_tile_table(s_proxy, 20, land_r)
      ...
      walk2(output_vars, output_vars_units, \(ov, ov_un) {
        ...
        mosaic(output_var = ov,
               ...
               years = yrs |> tail(-1), #|> tail(-1),
               prefix = str_glue("{rcm}_{gcm}_{dom}"),
               dir_dest_cloud = str_glue("gs://clim_data_reg_useast1/cordex/annual_aggregates/{ov}/"))
