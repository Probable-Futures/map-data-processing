...
for(dom in doms){
  ...
  for(i in 1:3){#seq_len(nrow(tb_models))){
    ...
    "cdo timpctl,85 {dir_cat}/step_1.nc -timmin {dir_cat}/step_1.nc -timmax {dir_cat}/step_1.nc {dir_cat}/step_2.nc" %>% 
      str_glue() %>% 
      system(ignore.stdout = T, ignore.stderr = T)
